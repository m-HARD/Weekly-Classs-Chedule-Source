        ...
        <table class="table-auto mt-10" v-for="(theClass,theClassIndex) in data.mainData.classes" :key="theClassIndex">
          ...
          <tbody>
            <tr class="flex flex-wrap border-b-2 border-gray-400" v-for="(day,dayIndex) in data.mainData.dayOfWeek" :key="dayIndex">
              ...
              <td class="w-56" :style="sub.subject.name != null ?{'background-color':colorDefulte == 0 ? data.mainData.colors[(sub.subject.id -1) %30]:colorDefulte == 1 ? data.mainData.colors[(sub.teacher.id -1) %30]:sub.teacher.id == 1  || sub.subject.name == customSubjectColor ? data.mainData.colors[0]:''}:''"
